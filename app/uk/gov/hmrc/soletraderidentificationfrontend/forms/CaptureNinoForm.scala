...
object CaptureNinoForm {
  val ninoRegex: Regex =
    "^([ACEHJLMOPRSWXY][A-CEGHJ-NPR-TW-Z]|B[A-CEHJ-NPR-TW-Z]|G[ACEGHJ-NPR-TW-Z]|[KT][A-CEGHJ-MPR-TW-Z]|N[A-CEGHJL-NPR-SW-Z]|Z[A-CEGHJ-NPR-TW-Y]) ?\\d{2} ?\\d{2} ?\\d{2} ?[A-D]{1}$".r

  val ninoNotEntered: Constraint[String] = Constraint("nino.not-entered")(nino =>
    validate(
  ...
  val ninoIncorrectFormat: Constraint[String] = Constraint("nino.incorrect-format")(nino =>
    validateNot(
