...
@Singleton
class RetrieveNinoIVStatusConnector @Inject() (httpClient: HttpClientV2, appConfig: AppConfig)(implicit ec: ExecutionContext) {

  def retrieveNinoIVStatus(journeyId: String)(implicit hc: HeaderCarrier): Future[BusinessVerificationStatus] =
    httpClient
      .get(url = url"${appConfig.getNinoIVResultUrl(journeyId)}")(hc)
      .execute[BusinessVerificationStatus](RetrieveIVStatusHttpReads, ec)
