...
class JourneyController @Inject()(controllerComponents: ControllerComponents,
  ...
  def createJourney(enableSautrCheck: Boolean): Action[JourneyConfig] = Action.async(parse.json[JourneyConfig] {
    json =>
      ...
      } yield JourneyConfig(continueUrl, businessVerificationCheck, PageConfig(optServiceName, deskProServiceId, signOutUrl, enableSautrCheck))
  }) {
    implicit req =>
  ...
  def createJourney(): Action[JourneyConfig] = Action.async(parse.json[JourneyConfig] {
    json =>
      ...
      } yield JourneyConfig(continueUrl, businessVerificationCheck, PageConfig(optServiceName, deskProServiceId, signOutUrl, enableSautrCheck.getOrElse(false)))
  }) {
    implicit req =>
      authorised().retrieve(internalId) {
        case Some(authInternalId) =>
