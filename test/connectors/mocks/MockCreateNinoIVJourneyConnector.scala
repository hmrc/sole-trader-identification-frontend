...
trait MockCreateNinoIVJourneyConnector extends MockitoSugar with BeforeAndAfterEach {
  self: Suite =>

  val mockCreateNinoIVJourneyConnector: CreateNinoIVJourneyConnector = mock[CreateNinoIVJourneyConnector]
  ...
  def verifyCreateNinoIVJourney(journeyId: String, nino: String, journeyConfig: JourneyConfig): Unit = {
    verify(mockCreateNinoIVJourneyConnector).createNinoIdentityVerificationJourney(
      ArgumentMatchers.eq(journeyId),
      ArgumentMatchers.eq(nino),
      ArgumentMatchers.eq(journeyConfig)
    )(ArgumentMatchers.any[HeaderCarrier])
